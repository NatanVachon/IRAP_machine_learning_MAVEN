...
######################################################################################################
def predict_period(begin_pred, end_pred, name="default"):
    pred_shocks = pd.DataFrame()
    pred_labels = pd.DataFrame()
    begins, ends = [], []

    print("Loading manager")
    manager = tm.loadManager("../Data/managers/MAVEN_V4_opti_3")
    ...
    for i in range(math.ceil(div)):
        ...
        print("Predicting")
        pred = corrected_shock_prediction(manager, data)
        ...
        for i in range(len(pred)):
            ...
            new_row = subsample_shock_predict(sample_b, aux_sample_b, sample_e, aux_sample_e)
        ...
        # Predict classes
        labels = subsample_class_predict(manager, data, [0.5 * (pred.at[k, "begin"] + pred.at[k, "end"]) for k in range(len(pred))])
        pred_labels = pd.concat([pred_labels, labels], ignore_index = True)


    # Save in df
    pred_shocks["begin"], pred_shocks["ends"] = begins, ends
    acom.save_df(pred_shocks, "../Data/amda_data/", name)

    return pred_shocks, pred_labels

def subsample_shock_predict(sample_b, aux_sample_b, sample_e, aux_sample_e):
...
"""
"""
def corrected_prediction(manager, dataset, dt_corr=70, plot=True, continuous=False):
...
def corrected_shock_prediction(manager, dataset, dt_corr=180, dt_shock=20*60):
    ...
    pred_data = corrected_prediction(manager, dataset, dt_corr)
